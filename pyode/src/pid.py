...
class PID:
    ...
    def __init__(self, P=2.0, I=0.0, D=1.0, Derivator=0, Integrator=0, Integrator_max=500, Integrator_min=-500, time = 0.0):
...
#         self.D_value = error - self.last_error
        self.D_value = self.Kd * ( self.error - self.Derivator)
        self.Derivator = self.error

        self.Integrator = float(self.Integrator + (self.error * float((time - self.last_time))))
